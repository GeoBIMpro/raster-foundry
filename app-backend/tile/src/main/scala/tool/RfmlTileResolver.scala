...
class RfmlTileResolver(db: Database, ec: ExecutionContext) extends LazyLogging {
  ...
  val ndtile = IntConstantTile(NODATA, 256, 256)
      ...
      exp match {
        ...
        case pr@ProjectRaster(projId, Some(band), celltype) =>
          ...
          futureSource.value.map({ maybeSource =>
            maybeSource match {
              case Some(tile) => Valid(TileLiteral(tile))
              case None => Valid(TileLiteral(ndtile))
            }
          })
      ...
      case sr@SceneRaster(sceneId, Some(band), celltype) =>
        Future.successful({
          Try {
            ...
            case Success(tile) =>
              Valid(TileLiteral(tile.band(band).interpretAs(celltype.getOrElse(tile.cellType))))
            case Failure(e) =>
